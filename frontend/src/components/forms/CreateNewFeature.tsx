  ...
  return (
    <>
      ...
      <select id="newFeature" {...register('type')}>
        ...
        <option key="import" value="import">
          import
        </option>
      </select>

      {selectedFeatureToCreate === 'import' && (
        <ImportFeature projectSlug={projectName || ''} callback={callback} />
      )}
            ...
            <details open>
              ...
              <select id="image_model" {...register('parameters.model')}>
                ...
                {(featuresOption['image-embeddings']?.models
                  ? Object.keys(
                      featuresOption['image-embeddings']['models'] as Record<string, unknown>,
                    )
                  : []
                ).map((element) => (
                  <option key={element} value={element}>
