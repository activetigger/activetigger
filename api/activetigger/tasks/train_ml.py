...
class TrainMLMultiClass(BaseTask):
    ...
    def __run(self) -> EventsModel:
        task_timer = TaskTimer(
            compulsory_steps=["setup", "train", "evaluate", "save_files"], optional_steps=["hyperparameters", "cv10"]
        )
        ...
        if self.optimize:
            task_timer.start("hyperparameters")
            ...
            self.model_params = {**self.model_params, **best_params}
            self.model, self.model_params, self.balance_classes = build_model(
                self.model_type, self.model_params, self.balance_classes
                )

            task_timer.stop("hyperparameters")
