...
class CreateProject(BaseTask):
    ...
    def build_trainset(self, dataset: pd.DataFrame) -> tuple[pd.DataFrame | None, list]:
        ...
        #============================Random (default / fallback)====================================================
            ...
            trainset = dataset.sample(self.params.n_train,random_state=self.random_seed)
        #====================== SAVE TRAINSET ======================
        if trainset is not None and len(trainset) > 0:
            cols_to_save=["id_external", "text"]+self.params.cols_context
            trainset[cols_to_save].to_parquet(self.params.dir.joinpath(self.train_file),index=True)
    ...
    def build_test_val_set(
        ...
        """
        Build valid and test sets with support for explicit start indices(in case of sequential/Determisitic) + overlap protection 
        """
        ...
        if train_rows:
            ...
            train_idx_set = set()
        #===================================SEQUENTIAL MODE (holdout_selection is None)=================================
        if self.params.holdout_selection is None:
            ...
            #1er cas : Validation overlaps with Train:
            ...
            #2eme cas :Test overlaps with Train:
            ...
            #3eme cas:Test overlaps with Validation:
        ...
        #=======================================STRATIFIED HOLD OUT========================================
            ...
            sampled_idx = df_grouped.apply(lambda x: x.sample(min(len(x), nb_per_cat), random_state=self.random_seed)).index.get_level_values(-1)
        ...
        #===================================RANDOM HOLD OUT (default)===============================================
            ...
            if n_test > 0 and n_val > 0:
                ...
                validset = holdout
        # ====================== SAVE TO PARQUET ======================
        for name, ds, file_attr, flag in [("valid", validset, "valid_file", "valid"),("test",  testset,  "test_file",  "test")]:
    ...
    def __call__(self) -> tuple[ProjectModel, pd.DataFrame | None, pd.DataFrame | None, pd.DataFrame | None]:
        """
        ...
        #=======================================================================
            ...
            # Then build train on the remaining rows
